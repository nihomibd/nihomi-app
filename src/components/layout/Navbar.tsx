        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden md:flex items-center space-x-2">
              ...
              {user ? (
                <div className="flex items-center gap-2">
                  {/* User Name Badge with Avatar */}
                  <div className="flex items-center gap-2 bg-slate-100 px-3 py-1.5 rounded-full border border-slate-200">
                    <div className="w-7 h-7 rounded-full overflow-hidden bg-red-600 text-white font-bold text-xs flex items-center justify-center shrink-0 shadow-sm relative border border-stone-200">
                      ...
                      <span>{(user?.user_metadata?.full_name || user?.name || user?.email || 'U').charAt(0).toUpperCase()}</span>
                    </div>
                    <span className="text-xs font-bold text-slate-800">
                      {user.user_metadata?.full_name || user.name || 'Learner'}
                    </span>
                  </div>
              ...
              <>
                ...
                <div className="p-3 bg-stone-50 rounded-2xl border border-stone-200 mb-2">
                  <div className="flex items-center space-x-2.5">
                    <div className="w-9 h-9 rounded-full overflow-hidden bg-red-600 text-white font-bold text-sm flex items-center justify-center shrink-0 shadow-sm relative border border-stone-200">
                      {user?.user_metadata?.avatar_url ? (
                        <img 
                          src={user.user_metadata.avatar_url} 
                          alt="" 
                      ...
                      <span>{(user?.user_metadata?.full_name || profile?.displayName || user?.name || 'U').charAt(0).toUpperCase()}</span>
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="text-xs font-bold text-stone-900 truncate">{user?.user_metadata?.full_name || profile?.displayName || user?.name || 'Learner'}</p>
